...
class ApplicationSpec extends FlatSpec with Matchers {
  ...
  "Application,index" should "send 404 on a bad request" in {
      running(FakeApplication()) {
        route(FakeRequest(GET, "/boum")) must beNone
      }
  }

  "Application" should "render an empty form on index" in {/*
      running(FakeApplication()) {
  ...
  "Application" should "send BadRequest on form error" in {/*
      running(FakeApplication()) {
  ...
  "Application" should "say hello" in {/*
      running(FakeApplication()) {
